class_name Player, "res://Player/icon.png"
extends KinematicBody2D
# 【文档说明】

# 本脚本用于Player的状态和行为的控制
# Player通过“公有函数”以感受外界的状态，
# 接受Input的输入以完成趋势转化到状态，
# 将以上的状态进行有机的混合，以完成动画的展现和物理的变化，
# 将行为作为信号/碰撞检测的鸭子方法进行输出


# 【信号】

signal change_life_num(damage)
signal die()
signal use_flight_props()
...
# 数据变量
export(int) var attack_a_damage = 2
export(int) var attack_b_damage = 4
export(PackedScene) var flight_props
export(int) var life_num = 5
export(int) var flight_props_num = 10
...
export(float,0,1) var jumpforce_slow_mode_rate = 0.25
export(float) var kill_check_time = 1.0

# 状态变量
export(bool) var is_alive = true
export(bool) var is_face_to_right = true
...
func _ready():
	...
	$Animation.flip_h = not is_face_to_right
...
func _physics_process(delta):
	_motion = self.move_and_slide(_motion, Vector2.UP, true, 10 )
...
func _make_damage(damage : int) -> void:
	if _life_num > 0:
		...
		emit_signal("change_life_num", damage)
...
func _on_AttackAreaLeft_body_entered(body):
	_bodys_left.append(body)


func _on_AttackAreaLeft_body_exited(body):
	_bodys_left.remove(_bodys_left.find(body))


func _on_AttackAreaRight_body_entered(body):
	_bodys_right.append(body)


func _on_AttackAreaRight_body_exited(body):
	_bodys_right.remove(_bodys_right.find(body))
